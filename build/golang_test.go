...
func TestGoGet(t *testing.T) {
	...
	testDataSource := filepath.Join("testdata", "golang")
	testDataTarget := filepath.Join(tempDirPath, "golang")
	assert.NoError(t, utils.CopyDir(testDataSource, testDataTarget, true, nil))

	service := NewBuildInfoService()
	goBuild, err := service.GetOrCreateBuild("build-info-go-test-golang", "2")
	assert.NoError(t, err)
	defer func() {
		assert.NoError(t, goBuild.Clean())
	}()
	goModule, err := goBuild.AddGoModule(filepath.Join(testDataTarget, "project"))
	assert.NoError(t, err)
	goModule.SetArgs([]string{"get", "github.com/golang/mock/mockgen@v1.4.1"})
	err = goModule.Build()
	assert.NoError(t, err)
	buildInfo, err := goBuild.ToBuildInfo()
	assert.NoError(t, err)
	assert.Len(t, buildInfo.Modules, 1)
	validateModule(t, buildInfo.Modules[0], 1, 0, "github.com/golang/mock", entities.Go, true)
}
